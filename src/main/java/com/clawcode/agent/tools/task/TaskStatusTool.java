...
public class TaskStatusTool implements Tool {
    ...
    public TaskStatusTool(TaskService taskService) {
    ...
    private static final ToolDefinition DEFINITION = new ToolDefinition(
        ...
        "Check the status of a previously submitted background task. "
            + "Returns an object with 'task_id', 'status', 'updated_at', and optionally 'error'.",
        Map.of(
            "type", "object",
            "properties", Map.of(
                "task_id", Map.of("type", "string",
                    "description", "ID of the task, as returned by task_submit.")
            ),
    ...
    public Mono<Object> execute(Object input, Object context) {
        ...
        return taskService.getStatus(taskId).map(TaskStatusTool::toMap);
    }
    ...
    private static Map<String, Object> toMap(TaskRecord task) {
        ...
        map.put("updated_at", task.updatedAt() != null ? task.updatedAt().toString() : null);
        if (task.error() != null) {
